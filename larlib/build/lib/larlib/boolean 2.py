...
def boundaryTriangulation(V,FV,EV,FE):
    ...
    for f,face in enumerate(FV):
        ...
        #model = (verts2D,[range(len(verts2D))],EW_)
        #struct = Struct([model])
        #U,EU = triangulation.structBoundaryModel(struct)
        U,EU = verts2D,EW_
        W_,FW_,EW_,polygons = inters.larFromLines([[U[u],U[v]] for u,v in EU])
        ...
        setsOfTriangles = triangulation.polygons2TriangleSet(W_,thePolygons)
        if setsOfTriangles != []:
...
def SBoundary3(W,EW,FW):
   ...
   #FE = [list(SB_2[:,f].tocoo().row) for f in range(SB_2.shape[1])]  # KO !!!
   import copy
   model = copy.deepcopy((W,FW,EW))
   EF_angle, ET,TW,FT = faceSlopeOrdering(model,FE)
   cellCount = -1
